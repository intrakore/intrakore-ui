<template>
  <div class="w-full space-y-[10px]">
    ...
    >
      <span v-if="props.label" class="text-base font-medium text-ink-blueprint-4">
        {{ props.label }}
      </span>
      <!-- Empty for alignment -->
      <span v-else></span>
      ...
      <span class="self-end" v-if="props.hint || $slots.hint">
        <slot name="hint">
          <span class="text-base font-medium text-ink-blueprint-4">
            {{ props.value }}%
          </span>
        </slot>
      </span>
    ...
    >
      ...
      <div
        ...
        class="h-full bg-surface-blueprint-7"
      ...
      <div
        ...
        :class="
          index <= filledIntervalCount
            ? 'bg-surface-blueprint-7'
            : 'bg-surface-blueprint-2'
        "
      ></div>
    </div>
  </div>
...
const indicatorContainerClasses = computed(() => {
  ...
  const layoutClasses = props.intervals
    ? 'flex space-x-1'
    : 'relative bg-surface-blueprint-2'
